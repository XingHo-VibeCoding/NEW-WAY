...
> `name` / `contact` 是 v1.5（2026-09-22）新增的两个**文本**字段，和上面两个数字字段共用同一格。存储上**不做特殊处理**：照原样 `JSON.stringify` 存进 `jiance.v1.profile`，读出来时用 `String(...)` 兜一道（防手改数据后拿到非字符串）。旧数据里没有这两个字段 = 视为空，不需要迁移（符合第十节 10.1「只加不改名」）。
...
| `entries` | 全部经历记录 | 数据本体 |
| `profile` | 个人参数（含 `name` / `contact`） | 数据本体 |
| `meta` | 首次使用时间等 | 数据本体 |
