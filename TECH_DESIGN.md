...
| 4 | **基本满足「断网可用」** | 除一款**在线字体**（思源宋体，`fonts.loli.net` 镜像，v1.9 起的唯一外链例外）外全程没有网络请求；断网时字体退回系统宋体，功能与数据零影响（对应 PRD 8.6，已随 v1.9 更新措辞） |
...
| `name` | 文本 | 否 | **姓名**；选填，不超过 20 字。**不参与任何计算**，只给简历预览页当抬头（PRD 5.3 / 8.5） |
| `contact` | 文本 | 否 | **联系方式**；选填，不超过 40 字，同样只给简历用 |
| `targetRole` | 文本 | 否 | **目标职位**；选填，不超过 20 字。**不参与任何计算**，只显示在底牌页的个人参数区（PRD 5.3） |
| `updatedAt` | 文本（时间） | 系统生成 | 最后修改时间 |

> `name` / `contact` 是 v1.5（2026-09-22）新增的两个**文本**字段，`targetRole` 是 v1.7（2026-09-23）新增的第三个**文本**字段，它们和上面两个数字字段共用同一格。存储上**不做特殊处理**：照原样 `JSON.stringify` 存进 `jiance.v1.profile`，读出来时用 `String(...)` 兜一道（防手改数据后拿到非字符串）。旧数据里没有这几个字段 = 视为空，不需要迁移（符合第十节 10.1「只加不改名」）。
...
| `entries` | 全部经历记录 | 数据本体 |
| `profile` | 个人参数（含 `name` / `contact` / `targetRole`） | 数据本体 |
| `meta` | 首次使用时间等 | 数据本体 |

**文件命名规则**：`jiance-backup-2026-09-20.json`（项目名 + 日期，方便你自己认）。

**四格都允许为空**（Day 7 拍板，2026-09-22）：`expenses` / `entries` / `profile` / `meta` 里任何一格都可以是空的，一份「什么都没有」的备份**算合法输入**，导入时正常收下、不报错（详见 10.4）。校验只管**结构对不对**（`format` / `version`），不管**内容多不多**。
...
| 手机上看 | **可看可用**（v1.8 更新：原「本期不支持」已过时 —— Day 7 起做了窄屏断点，Day 9 补充任务完善为响应式） | 同一份 `index.html` 自适应：≥900px 双栏、<900px 单列（640 / 380 两个窄屏断点）；**数据仍不跨设备**，换设备照旧走 10.3 上面的备份文件搬运 |
...
| 收下空备份（等价于手动清空全部数据） | 不把「备份里没内容」当错误拦下 |

**两条导入边界口径（Day 7 拍板，2026-09-22）：**

| # | 口径 | 为什么要定死 |
|---|---|---|
| 1 | 导入成功后，流水列表**跳到「备份里最新一条记录所在的月份」**，不是跳回当前月 | 备份里的记录可能都在 8 月，而打开页面时是 9 月。跳回当前月会看到一张空列表，用的人会以为「导入失败了」——明明成功却像失败，是最容易引发误判的一种反馈 |
| 2 | **空备份是合法输入**（四格都为空也照收），两个列表回空态引导 | 它等价于「手动清空全部数据」，是用户可能真想要的操作。把它判成错误，反而堵死了一条正当路径 |

> 落点口径只影响**导入之后停在哪个画面**，不动数据本身：`Store` 里写进去的四格与备份文件逐字节一致（导出再导入一圈不变形是回归项之一）。
...
>
> 2026-09-22（Day 7）：新增一处同步 —— **删掉经历记录的 `tags` 字段**（已同步改 PRD 5.2 字段表、功能 2 用户动作、验收 8.4 / 8.5），两份文档同时升 **v1.3**。原因：Day 7 任务清单把「经历标签」列入不做项。
>
> 2026-09-22（Day 7，第 2 次）：再同步一处**口径调整** —— 「月」拆成两个：**记一笔页**列表上方的合计改为**跟随列表看的月份（当月）**，**底牌页**的本月三数仍按**当前自然月（本月）**。已同步改 PRD 功能 1（用户动作 6 + 可观察结果）、第五节（新增两个口径对照表）、功能 3、页面地图、验收 8.3（加 1 条），两份文档同时升 **v1.4**。原因：Day 7 实现完成后拍板 —— 原口径「合计数字与列表在看哪个月无关」用起来反直觉。**本文档正文无实质改动**（字段、数据流、技术选型都没变），只是登记这次同步。
>
> 2026-09-22（Day 7，第 3 次）：新增一处同步 —— 个人参数**新增 `name` / `contact` 两个选填文本字段**（已同步改 PRD 5.3 字段表与 v1.5 说明、功能 3 用户动作 2、第四节页面地图第 4 页、验收 8.2 / 8.5），两份文档同时升 **v1.5**。原因：简历预览页此前没有任何身份信息，打印出来是一份「不知道是谁的」简历（Day 7 拍板）。连带一起定的展示口径：**量化结果在界面上统一显示成「成果：xxx」** —— 改的只是前端渲染（前缀拼在 `resultLabel()` 一处），不落库、不动 `result` 字段本身。
>
> 2026-09-23（Day 8）：新增一处同步 —— 个人参数**新增 `targetRole`（目标职位）一个选填文本字段**（已同步改 PRD 5.3 字段表与 v1.7 说明、功能 3 用户动作 2、验收 8.2），两份文档同时升 **v1.7**。原因：底牌页只回答「钱还能撑多久」，少了「我在往哪个方向走」这一半（Day 8 拍板）。**本文档正文只动 5.3 字段表与 5.5 的 `profile` 说明** —— 存储结构没变（仍是同一格 `jiance.v1.profile`）、备份格式没变（仍是同一个 `profile` 键），因此**不需要任何数据迁移**。
>
> 下面这张表记的是 **v1.2 那次**同步的内容；两份文档的当前口径，一律以正文为准。
...
**入口位置已定**（2026-09-20）：放**底牌页「设置个人参数」区**。理由：数据管理属于「设置」性质，且是低频操作，不该挤进导航条。已同步写进 PRD 第四节。
...
| 10 | 导出 / 导入的入口位置 | **已定**：底牌页「设置个人参数」区（2026-09-20） |
| 11 | 经历记录要不要 `tags`（技能标签） | **已定**：本期不做（2026-09-22 Day 7 拍板）。PRD 升 v1.3 已同步删除该字段；以后要加，按 10.1「只加不改名」原则追加，不用改已有数据 |
| 12 | 两个核心模块（账簿面 / 简历面）各含哪些页面 | **已定**（2026-09-22）：账簿面 = 底牌页 + 记一笔；简历面 = 记一条 + 简历预览。模块名只作归属标识，页面名不变。已写进 `AGENTS.md` 附三第 2 条 |
| 13 | 记一笔页那两个合计数字，算的是哪个月 | **已定**（2026-09-22 Day 7 拍板）：**跟随流水列表正看着的那个月**（切到 8 月，合计就是 8 月的；标题同步显示「8月收入 / 8月支出」）。**底牌页**的本月收入 / 支出 / 结余仍按**当前自然月**，两者口径不同，见 PRD 第五节「『月』的两个口径」。PRD 已升 v1.4 同步 |

---

*本文档由 Day 5 任务产出；v1.3 / v1.4 于 Day 7（2026-09-22）随拍板同步。不含可运行代码，写代码从第 2 周开始。*
