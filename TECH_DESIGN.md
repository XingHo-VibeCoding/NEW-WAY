...
> 版本：v1.1 ｜ 日期：2026-09-20（Day 5）
> 修订记录：v1.0 初稿 → v1.1 按秋鹰师拍板修订（新增 `updatedAt` 字段、经历记录改为起止时间、新增导出/导入数据功能、确认单文件结构、确认改写 `index.html`、数据流图入库）
> 上游文档：`PRD.md`（Day 4 需求）、`research.md`（Day 3 研究）
> 本文回答：「用哪些零件拼、数据从哪来、到哪去、出错了怎么办」
> 本文**不回答**：「每个页面长什么样、按钮点下去的具体反馈」（那在 PRD）
> 本期状态：**只有设计，不含任何可运行代码**；写代码从第 2 周开始（以当天清单为准）
> **与 PRD 不一致的地方全部列在第十一节** —— 那是这次拍板的改动，PRD 待同步
...
**文件是明文的**：用记事本打开就能看懂里面所有内容。这一点在第十二节列为待确认项。
...
| 导入前提示先备份当前数据 | 不做备份文件加密（列为待确认项） |

---

## 十一、与 PRD 的差异（诚实标注）

> 本次拍板的改动**没有回头改 PRD**（对齐清单「今日不做：回头大改 PRD」）。列在这里，等后续同步。

| # | PRD 的写法 | 本文档口径 | 原因 |
|---|---|---|---|
| 1 | 经历记录只有一个 `date` 字段 | 改为 `startDate` + `endDate`（`endDate` 可空） | 秋鹰师 2026-09-20 拍板「要起止时间」 |
| 2 | MVP 只含三个功能（记一笔 / 记一条 / 看底牌） | **新增第四个功能：导出 / 导入数据** | 秋鹰师拍板，防数据永久丢失 + 换设备搬数据 |
| 3 | 字段表未提 `updatedAt` | 三组数据都加 `updatedAt` | 为将来上后端做同步预留 |
| 4 | PRD 未指定项目结构的文件数量 | 确认为单个 `index.html` | 与 PRD 第七节一致，无需改动 |
| 5 | PRD 未说明 Day 2 占位页的去向 | `index.html` 直接改写 | 秋鹰师拍板 |

**需要后续动作**：第 1、2、3 条建议在写代码之前（第 2 周开始前）同步进 PRD，否则会出现「两份文档打架」。
...
| 6 | 把上面 5 条同步进 PRD | **待办**，建议第 2 周开始前做 |
| 7 | 备份文件要不要加密 | 未定。备份是**明文** `.json`，用记事本就能看全部内容。本期建议不加密（数据本来就只在自己电脑上），**待秋鹰师确认** |
